...
export default function AuditLogsView({ logs, clearLogs, loadLogs, loadMoreLogs, hasMore }: AuditLogsViewProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [expandedLog, setExpandedLog] = useState<string | null>(null);
  const [isConfirmingClear, setIsConfirmingClear] = useState(false);
  const [hasRequestedLoad, setHasRequestedLoad] = useState(logs.length > 0);
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 space-y-6">
      ...
      {!hasRequestedLoad ? (
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center p-8 text-center shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Consulta de Auditoría Diferida</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md mb-6">
            Para optimizar el rendimiento y ahorrar lecturas en Firebase, los registros históricos no se cargan de forma automática.
          </p>
          <button
            onClick={handleLoadLogs}
            disabled={isLoading}
            className="flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white rounded-xl text-sm font-medium transition-colors shadow-md shadow-indigo-200 dark:shadow-none"
          >
            {isLoading ? 'Cargando...' : 'Cargar últimos 50 registros'}
          </button>
        </div>
      ) : (
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col min-h-0 shadow-sm">
          ...
          {hasMore && (
            <div className="flex-none p-4 border-t border-gray-200 dark:border-gray-700 flex justify-center bg-gray-50/50 dark:bg-gray-900/10">
              <button
                onClick={handleLoadMoreLogs}
                disabled={isMoreLoading}
                className="px-6 py-2 border border-gray-200 dark:border-gray-700 hover:border-indigo-500 dark:hover:border-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-400 text-gray-700 dark:text-gray-300 rounded-xl text-sm font-medium transition-all"
              >
                {isMoreLoading ? 'Cargando...' : 'Cargar 50 registros más'}
              </button>
            </div>
          )}
        </div>
