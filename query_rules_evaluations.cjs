...
async function run() {
  console.log("Querying GCP Monitoring for rules/evaluation_count...");
  const endTime = new Date();
  const startTime = new Date(endTime.getTime() - 7 * 24 * 3600 * 1000); // 7 days
  
  const filter = encodeURIComponent('metric.type="firestore.googleapis.com/rules/evaluation_count"');
  ...
  try {
    ...
    if (response.timeSeries && response.timeSeries.length > 0) {
      ...
      response.timeSeries.forEach((ts, idx) => {
        ...
        console.log(`Points count: ${points.length}, Sum of evaluations: ${sum}, Max: ${max}`);
      });
    } else {
      console.log("No timeSeries found for rules/evaluation_count.");
    }
  } catch (e) {
